...
%Load Timestamps
F_ts=readtable("WiFi - 3.xlsx",'Sheet',"Fermi",'Range',"A:A");
M_ts=readtable("WiFi - 3.xlsx",'sheet',"Majorana",'Range',"A:A");
P_ts=readtable("WiFi - 3.xlsx",'sheet',"Pontecorvo",'Range',"A:A");
...
%Load RSSI values
F_rssi=xlsread("WiFi - 3.xlsx","Fermi","B:B");
M_rssi=xlsread("WiFi - 3.xlsx","Majorana","B:B");
P_rssi=xlsread("WiFi - 3.xlsx","Pontecorvo","B:B");
...
fileName = strcat('garden - 3 red.png'); %kitchen's roof plan
...
for k = 1:length(F_cm)
    ...
    cplot(F_cm(k),289,162); %apply trilat method
    cplot(M_cm(k),140,346);
    cplot(P_cm(k),127,22);
    ...
    xlim([0 389]) %set xlim to zoom into the bg img
    ylim([0 371]) %set ylim to zoom into the bg img
